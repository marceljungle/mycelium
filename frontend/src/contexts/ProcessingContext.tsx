...
import type { ProcessingResponse } from '@/server_api/client';
...
export function ProcessingProvider({ children }: ProcessingProviderProps) {
  ...
  // Processing state - no localStorage persistence, API is source of truth
  ...
  const fetchProgress = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Update processing state based on API response (API is source of truth)
        if (data.is_processing) {
          setProcessLoading(true);
          setProgressInfo({ 
            stage: 'processing',
            current: data.processed_tracks,
            total: data.total_tracks
          });
        } else {
          // Only clear processing state if we're currently showing it as processing
          if (processLoading) {
            setProcessLoading(false);
            setProgressInfo(null);
            // Only show completion message if processing was actually running
            if (operationMessage?.includes('started') || operationMessage?.includes('Processing')) {
